...
echo "\n************************************"
log_info "Configuring Identity Service ..."
echo "************************************"
...
create_uer() {
  log_info "Creating user '$DEFAULT_USERNAME' in Identiy Service ..."
  # create the user first
  STATUS_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$IDS_BASE_URL/auth/admin/realms/$REALM/users" \
  ...
  if [ "$STATUS_CODE" = "201" ]; then
    ...
    log_error "Couldn't create the user: '$DEFAULT_USERNAME'. Failed with status code: $STATUS_CODE"
  fi

  # Get the user details in order to extract the ID
  USER_ID=$(curl -s "$IDS_BASE_URL/auth/admin/realms/$REALM/users?briefRepresentation=true&first=0&max=1&search=$DEFAULT_USERNAME" \
    -H "Authorization: Bearer $TOKEN" | jq -r ".[0].id")

  log_info "Retrieved the user_id '$USER_ID' for '$DEFAULT_USERNAME'."

  # Set the user password
  STATUS_CODE=$(curl -s -o /dev/null -w "%{http_code}" --request PUT "$IDS_BASE_URL/auth/admin/realms/$REALM/users/$USER_ID/reset-password" \
    -H "Content-Type: application/json;charset=utf-8" \
    -H "Authorization: Bearer $TOKEN" \
    -d "{
        \"type\": \"password\",
  ...
  if [ "$STATUS_CODE" = "204" ]; then
    log_info "The user password '$DEFAULT_USERNAME' has been set successfully."
  else
    log_error "Couldn't update the user $DEFAULT_USERNAME's password. Failed with status code: $STATUS_CODE"
  fi
...
if [ "$AUTH0_CLIENT_ID" = "not-set" ]; then
  log_info "Auth0 client Id has not been set, so no external IdP will be configured as a SAML provider."
  create_uer
else
  log_info "Configuring SAML with Auth0 as an external IdP....."
  configure_saml
fi
