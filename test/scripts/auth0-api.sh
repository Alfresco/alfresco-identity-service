...
{
  "name": "${app_name}",
  "description": "Client for Keycloak testing",
  ...
  "addons": {
     "samlp": {
       ...
       "logout": {
         "callback": "${kc_base_url}/auth/realms/alfresco/broker/saml/endpoint",
         "slo_enabled": true
       },
  ...
  "callbacks": [
    "${kc_base_url}/auth/realms/alfresco/broker/saml/endpoint"
  ],
...
create)
  check_var "${app_name}" "app_name (2nd)"
  check_var "${kc_base_url}" "kc_base_url (3rd)"
  create_app
  ;;
delete)
  check_var "${app_name}" "app_name (2nd)"
