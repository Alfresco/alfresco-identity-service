...
log_info "Getting the expected Keycloak version from: ${KC_HOME}"
EXPECTED_VERSION=$(< "${KC_HOME}/version.txt"  cut -f4 -d' ')
log_info "Got expected Keycloak version as: ${EXPECTED_VERSION}"

get_token
ACTUAL_VERSION="$(curl -s "${KC_BASE_URL}/auth/admin/serverinfo" -H "Authorization: Bearer ${TOKEN}" | jq -r ".systemInfo.version")"
